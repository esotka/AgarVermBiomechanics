...
m <- lmer(log(tmp[,"sumlength"])~latitude*Continent.short+(1|site), data=tmp)
print("all thalli")
...
pdf("output/Length-byLatitude-prettyPlot.pdf",width=5,height=5)
### plot - 3 continents
### all thalli
tmp <- arch[arch$Length=="Y" & ! arch$Continent.short=="wNA",]
...
fit <- lm(sumlength~Continent.short*latitude,data=tmp2)
