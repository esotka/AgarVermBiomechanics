...
m <- lmer(log(tmp[,"sumprojarea"])~latitude*Continent.short+(1|site), data=tmp)
print("all thalli")
...
pdf("output/SurfaceArea-byLatitude-prettyPlot.pdf",width=5,height=5)
### plot - 3 continents
### all thalli
tmp <- arch[arch$Surfacearea=="Y" & ! arch$Continent.short=="wNA",]
tmp <- melt(tmp[,c("site","natnon","Continent.short","sumprojarea")])
tmp2 <- cast(tmp,site+natnon+Continent.short~variable,mean,na.rm=T)
tmp2$latitude <- meta$lat[match(tmp2$site,meta$field_site_code_2015)]
fit <- lm(sumprojarea~Continent.short*latitude,data=tmp2)
visreg(fit,"latitude",by="Continent.short")#,overlay=T)
